...
Một tối ưu quan trọng ở tầng backend là phân biệt nghiêm ngặt giữa hai loại lời gọi chaincode: \pkg{invoke} tạo ra giao dịch ghi và phải qua orderer để đóng khối; \pkg{query} chỉ đọc dữ liệu trực tiếp từ World State của peer, không tạo giao dịch và không tốn quota khối. Bảng~\ref{tab:invoke-vs-query} liệt kê cách phân loại các hàm chaincode chính của hệ thống.
...
\hline
\pkg{SubmitVote}, \pkg{CommitMerkleRoot}, \pkg{RevealVoteCompact} & \pkg{invoke} & Có & 2--3 s \\
\hline
\pkg{GetVote}, \pkg{GetMerkleRoot}, \pkg{GetTally}, \pkg{GetAuditCounts}, \pkg{VerifyVoteReceipt} & \pkg{query} & Không & dưới 100 ms \\
\hline
\end{tabularx}
\end{table}
