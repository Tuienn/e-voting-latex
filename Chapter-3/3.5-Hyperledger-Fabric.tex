...
Hệ thống bầu cử yêu cầu hai đặc tính trái ngược với blockchain công khai: xác minh danh tính của từng node và tính chung quyết xác định (deterministic finality). Một mạng công khai như Ethereum không thỏa mãn cả hai -- bất kỳ ai cũng có thể tham gia mạng và trở thành node, và các khối có thể bị thay thế tạm thời bởi nhánh dài hơn (hiện tượng fork). Hyperledger Fabric khắc phục cả hai điểm yếu này bằng kiến trúc được cấp phép (permissioned) và cơ chế đồng thuận xác định Raft. Bảng~\ref{tab:ethereum-vs-fabric} so sánh các tiêu chí then chốt.
...
\hline
Độ trễ chung quyết & 6--60 giây & dưới 1 giây (deterministic) \\
\hline
Chi phí & Phí gas & Không có (hạ tầng riêng) \\
\hline
Dữ liệu & Công khai & Riêng tư theo channel \\
\hline
