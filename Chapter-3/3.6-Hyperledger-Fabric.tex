...
Hệ thống bầu cử yêu cầu hai đặc tính trái ngược với blockchain công khai: \textbf{xác minh danh tính} của từng node và \textbf{tính chung quyết xác định} (\textit{deterministic finality}). Một mạng công khai như Ethereum không thỏa mãn cả hai — bất kỳ ai cũng có thể trở thành node, và các khối có thể bị thay thế tạm thời bởi nhánh dài hơn (hiện tượng \textit{fork}). Hyperledger Fabric khắc phục cả hai điểm yếu này bằng kiến trúc được cấp phép (\textit{permissioned}) và cơ chế đồng thuận xác định Raft. Bảng~\ref{tab:ethereum-vs-fabric} so sánh các tiêu chí then chốt.
