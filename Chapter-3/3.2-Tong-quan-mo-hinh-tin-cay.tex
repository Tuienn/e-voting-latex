...
\begin{itemize}
    ...
    \item \textbf{Tầng 3 -- Quản lý phiên trên Redis:} mỗi phiên bỏ phiếu được lưu với thời gian sống ngắn, gắn cờ \pkg{signed}/\pkg{voted} và được dùng làm máy trạng thái phía máy chủ.
    ...
    \item \textbf{Tầng 5 -- Ràng buộc MongoDB:} chỉ mục duy nhất trên cặp \pkg{(electionId, voterId)} và \pkg{(electionId, blindedCommitment)}, cùng các giao dịch nhiều thao tác (multi-document transaction), ngăn lỗi đua tranh và phiếu trùng.
    ...
    \item \textbf{Tầng 7 -- Mạng blockchain Hyperledger Fabric:} mạng riêng được cấp phép, có chứng chỉ X.509 cho mỗi node, hỗ trợ chính sách xác nhận giao dịch (endorsement policy) và tính bất biến đảm bảo bởi liên kết băm khối liên tiếp.
\end{itemize}
...
\hline
Tính duy nhất (\textit{one voter -- one vote}) & Phiên Redis kết hợp chỉ mục duy nhất trên MongoDB và cơ chế chống ký trùng lặp tại node ký \\
\hline
...
\hline
Tính ẩn danh (\textit{unlinkability}) & Bảng \pkg{Vote} và \pkg{RevealedVote} không thể nối kết qua bất kỳ trường khóa nào \\
\hline
\end{tabularx}
\end{table}
