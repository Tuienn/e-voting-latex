...
Hệ thống dùng thư viện \texttt{merkletreejs} ở phía off-chain (Node.js) và mã Go ở phía on-chain (chaincode). Để cây sinh ra ở hai phía giống hệt nhau, cấu hình của \texttt{merkletreejs} bật ba tùy chọn quan trọng (Hình~\ref{fig:merkle-tree-config}):
...
\begin{itemize}
    ...
    \item \textbf{Đối chiếu chéo DB và chain:} giá trị \texttt{election.merkleRoot} lưu trong MongoDB được đối chiếu với \texttt{MerkleRootView.merkleRoot} đọc từ chain. Bất kỳ chênh lệch nào cũng đồng nghĩa backend đã tự ý thay đổi root sau khi commit — sự việc có thể bị bất kỳ bên kiểm toán nào phát hiện.
\end{itemize}
