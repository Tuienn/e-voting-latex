...
\begin{itemize}
    \item \textbf{\pkg{sortLeaves}:} sắp xếp các lá trước khi xây cây -- cây trở thành xác định, không phụ thuộc vào thứ tự phiếu được nộp.

    \item \textbf{\pkg{sortPairs}:} sắp xếp cặp anh em trước khi băm -- nhờ vậy bằng chứng (\textit{proof}) không cần kèm vị trí trái/phải.

    \item \textbf{\pkg{duplicateOdd}:} nếu số lá lẻ, sao chép lá cuối -- đảm bảo cây luôn hoàn chỉnh.
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth, keepaspectratio]{Figures/code/merkle-tree-config.png}
    \caption{Cấu hình MerkleTree của thư viện merkletreejs ở tầng off-chain}
    \label{fig:merkle-tree-config}
\end{figure}

Tùy chọn \pkg{sortPairs} phải được phản chiếu nguyên vẹn ở phía chaincode. Hàm \pkg{hashSortedPair} trong chaincode (Hình~\ref{fig:hash-sorted-pair-go}) làm chính xác việc đó: so sánh hai dãy byte, gom dãy nhỏ hơn về trước, rồi băm SHA-256 phần nối. Nếu thuật toán này khác giữa hai phía, gốc Merkle on-chain và off-chain sẽ lệch ngay cả với cùng tập phiếu -- và toàn bộ luồng kiểm chứng sau đó vô hiệu.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth, keepaspectratio]{Figures/code/hash-sorted-pair-go.png}
    \caption{Hàm hashSortedPair trong chaincode Go đồng bộ với off-chain}
    \label{fig:hash-sorted-pair-go}
\end{figure}

\subsubsection{Cam kết gốc Merkle đảm bảo gì}

Sau khi \pkg{commitMerkleRoot} được ghi lên sổ cái Fabric, ba bất biến quan trọng được thiết lập:

\begin{itemize}
    \item \textbf{Snapshot bất biến:} tập hợp phiếu tại thời điểm đóng bầu cử được \emph{đóng băng} trong một giá trị 32 byte. Bất kỳ thay đổi nào trong tập phiếu (thêm, xóa, sửa) đều sinh ra root khác.
    ...
    \item \textbf{Đối chiếu chéo cơ sở dữ liệu và blockchain:} giá trị \pkg{election.merkleRoot} lưu trong MongoDB được đối chiếu với \pkg{MerkleRootView.merkleRoot} được đọc từ blockchain. Bất kỳ chênh lệch nào cũng đồng nghĩa backend đã tự ý thay đổi root sau khi commit -- sự việc có thể bị bất kỳ bên kiểm toán nào phát hiện.
\end{itemize}
