...
Mỗi lá phiếu cho phép cử tri chọn từ một đến \pkg{maxSelectableCandidates} ứng viên. Để chữ ký vẫn xác thực được bất kể thứ tự lựa chọn, thông điệp $M$ không gắn trực tiếp từng \pkg{candidateId} mà gắn một chuỗi \textit{canonical}: $\textit{payload} = \texttt{JSON.stringify}(\text{canonical}(\textit{candidateIds}))$ với $\text{canonical}(ids) = [\,\text{...new Set}(ids)\,].\text{sort}()$ — loại bỏ phần tử trùng lặp rồi sắp xếp theo từ điển. Nhờ quy ước này, máy khách (lúc ký) và dịch vụ \pkg{reveal-vote} (lúc xác thực) luôn dựng lại cùng một chuỗi byte thông qua hàm \pkg{buildVoteMessage} trong Hình~\ref{fig:build-vote-message}; chỉ cần lệch một byte thì phép kiểm tra $h_{check} = h$ sẽ thất bại.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth, keepaspectratio]{Figures/code/build-vote-message-canonical.png}
    \caption{Hàm \pkg{buildVoteMessage} dựng thông điệp $M$ từ \pkg{electionId} và tập \pkg{candidateIds} đã chuẩn hóa (canonicalize)}
