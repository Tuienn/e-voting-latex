...
Toàn bộ lớp mật mã của hệ thống được xây dựng quanh lược đồ chữ ký mù tập thể EC-Schnorr trên đường cong elliptic \texttt{secp256k1}. Đây là kết quả cân nhắc giữa hai tiêu chí thực tế: tính \textit{linearity} của Schnorr cho phép tổng hợp chữ ký không tương tác, đồng thời kích thước scalar và point trên \texttt{secp256k1} nhỏ hơn nhiều so với khóa DSA/RSA cùng mức an toàn. Bảng~\ref{tab:so-sanh-schnorr} đối chiếu một số tiêu chí then chốt giữa hai họ lược đồ.
...
\begin{itemize}
    \item \textbf{Tính mù (\textit{Blindness}):} máy chủ chỉ thấy giá trị \textit{thử thách đã làm mù} $r$ và \textit{cam kết đã làm mù} dưới dạng băm $\text{SHA256}(C')$. Không tồn tại cách thức tính toán hiệu quả để phục hồi \texttt{candidateId} hay cặp $(\alpha, \beta)$ từ các giá trị này.
    ...
    \item \textbf{Chống tấn công phát lại liên bầu cử (\textit{Cross-election replay prevention}):} thông điệp $M$ được gắn định danh bầu cử qua \textit{domain separator} \textit{``ev-vote-v1''} kết hợp \texttt{electionId}. Một chữ ký hợp lệ trong bầu cử $A$ khi xác thực trong bầu cử $B$ sẽ thu được $h_{check} \neq h$ vì thông điệp $M$ khác.
\end{itemize}

Toàn bộ thư viện \texttt{ec-schnorr} của hệ thống được hiện thực dựa trên \texttt{@noble/curves} (\texttt{secp256k1}) và \texttt{@noble/hashes} (SHA-256) — các thư viện cài đặt theo thời gian hằng (\textit{constant-time}), không phụ thuộc native binding và chạy được trên cả Node.js lẫn trình duyệt, qua đó cho phép cử tri tự kiểm chứng chữ ký bằng cùng một mã nguồn ngay tại máy của mình.
