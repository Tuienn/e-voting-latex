\subsection{Nền tảng blockchain Hyperledger Fabric}
...
\begin{itemize}
    \item \textbf{Mạng có cấp phép (Permissioned Network):} Mọi thực thể tham gia mạng -- bao gồm người dùng, ứng dụng khách, node xác thực và node sắp xếp giao dịch -- đều phải được định danh thông qua chứng chỉ số do Cơ quan cấp chứng chỉ (Certificate Authority -- CA) phát hành. Đặc tính này loại bỏ sự ẩn danh, hỗ trợ việc thiết lập trách nhiệm pháp lý một cách minh bạch, đồng thời đáp ứng các tiêu chuẩn tuân thủ nghiêm ngặt trong môi trường doanh nghiệp.

    \item \textbf{Kiến trúc mô-đun (Modular Architecture):} Hyperledger Fabric được thiết kế theo hướng các thành phần có thể \textit{tích hợp linh hoạt} (pluggable), bao gồm cơ chế đồng thuận, hệ thống quản lý danh tính, cơ sở dữ liệu trạng thái và môi trường thực thi hợp đồng thông minh. Tính mô-đun này cho phép các tổ chức tùy biến nền tảng theo nhu cầu nghiệp vụ thay vì phải chấp nhận một mô hình cứng nhắc.

    \item \textbf{Hiệu năng và khả năng mở rộng (High Performance and Scalability):} Quá trình thực thi hợp đồng thông minh được tách biệt khỏi quá trình đồng thuận, giúp các giao dịch được xử lý song song trên nhiều node và sau đó mới được sắp xếp theo thứ tự. Cách tiếp cận này giúp Hyperledger Fabric đạt được tốc độ xử lý hàng nghìn giao dịch mỗi giây với độ trễ chỉ ở mức vài trăm ms, vượt trội so với các blockchain công khai sử dụng cơ chế đào (mining).
...
\textbf{Ưu điểm:} Là một nền tảng blockchain liên hợp được thiết kế dành riêng cho môi trường doanh nghiệp, Hyperledger Fabric mang lại một số lợi thế nổi bật:

\begin{itemize}
    \item Cung cấp khả năng bảo mật và phân quyền dữ liệu hiệu quả nhờ cơ chế Channel và Private Data Collection, đáp ứng tốt các kịch bản hợp tác giữa nhiều bên với mức độ tin cậy không đồng đều.
...
\textbf{Hạn chế:} Bên cạnh các ưu điểm, Hyperledger Fabric cũng tồn tại một số hạn chế cần được lưu ý khi triển khai:

\begin{itemize}
    \item Kiến trúc hệ thống tương đối phức tạp với nhiều thành phần phối hợp (Peer, Orderer, CA, MSP, Channel, chaincode, v.v.), đòi hỏi thời gian tìm hiểu đáng kể đối với các nhóm phát triển mới làm quen.
