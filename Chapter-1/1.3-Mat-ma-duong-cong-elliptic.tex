...
\label{sec:mat-ma-duong-cong-elliptic}
...
\begin{itemize}
    ...
    \item \textbf{Tính kháng va chạm (Collision Resistance):} không tồn tại thuật toán hiệu quả tìm được hai thông điệp khác nhau $M_1 \neq M_2$ thỏa mãn $H(M_1) = H(M_2)$.
\end{itemize}
...
\textbf{Chữ ký số:} Một lược đồ chữ ký số (Digital Signature) được xác định bởi bộ ba thuật toán $(\textsf{Gen}, \textsf{Sig}, \textsf{Ver})$ trên nền hệ mật khóa công khai:

\begin{itemize}
    \item \textbf{Sinh khóa (\textsf{Gen}):} nhận tham số an toàn $1^k$, sinh ra cặp khóa công khai -- bí mật $(pk, sk)$.

    \item \textbf{Ký (\textsf{Sig}):} sử dụng khóa bí mật $sk$ để tạo chữ ký $s \leftarrow \textsf{Sig}(sk, M)$ tương ứng với thông điệp $M$. Trên thực tế, người ký không ký trực tiếp lên $M$ mà ký lên giá trị băm $H(M)$.

    \item \textbf{Xác thực (\textsf{Ver}):} sử dụng khóa công khai $pk$ để kiểm tra tính hợp lệ của cặp $(M, s)$, trả về kết quả thuộc $\{0, 1\}$.
\end{itemize}
