...
Toàn bộ trạng thái của chaincode được lưu vào \textit{World State} của Fabric thông qua khóa tổ hợp (\textit{composite key}) do hàm \pkg{CreateCompositeKey} tạo ra. Mỗi loại đối tượng có một tiền tố riêng nhằm phân tách không gian tên (\textit{namespace}) và thuận tiện cho việc duyệt theo từng nhóm.
...
\noindent Tổng số phiếu, số phiếu đã giải mù và kết quả kiểm phiếu theo ứng viên đều được tổng hợp theo yêu cầu bằng truy vấn khoảng (\pkg{GetStateByPartialCompositeKey}) trực tiếp trên bản ghi gốc. Thiết kế này loại bỏ hoàn toàn xung đột \textit{MVCC\_READ\_CONFLICT} khi nhiều cử tri nộp phiếu hoặc giải mù đồng thời -- vì mỗi giao dịch chỉ ghi đúng một khóa riêng biệt mà không đọc hay ghi bất kỳ khóa chung nào.
...
Hàm \pkg{ComputeRevealPayloadHash} là hàm tiện ích thuộc loại \textit{query}, cho phép tính lại giá trị \pkg{revealPayloadHash} từ bộ ba $(\pkg{candidateIdsJson}, h, s')$, qua đó cho phép bên thứ ba kiểm tra giá trị băm được lưu trên blockchain có khớp với dữ liệu thực hay không.
