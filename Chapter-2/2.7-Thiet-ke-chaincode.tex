...
Hàm \pkg{SubmitVote} thuộc loại \textit{invoke}, được Coordinator Service gọi mỗi khi một cử tri nộp phiếu. Trước khi ghi nhận, chaincode kiểm tra cuộc bầu cử chưa cố định gốc Merkle và \pkg{voteID} chưa tồn tại nhằm chặn trùng lặp; sau khi ghi thành công, bộ đếm \pkg{TotalVoteCount} trong đối tượng \pkg{Stats} được tăng lên một đơn vị.
...
Hàm \pkg{CommitMerkleRoot} thuộc loại \textit{invoke}, được Coordinator Service gọi đúng một lần khi quản trị viên đóng cuộc bầu cử. Chaincode đối chiếu \pkg{voteCount} truyền vào với giá trị \pkg{TotalVoteCount} đã tích lũy trên blockchain nhằm phát hiện sai lệch giữa cơ sở dữ liệu off-chain và sổ cái; nếu khớp, gốc Merkle được lưu lại kèm cờ \pkg{committed = true} và không thể bị ghi đè.
...
Hàm \pkg{VerifyVoteReceipt} thuộc loại \textit{query}, đóng vai trò điểm tin cậy cuối cùng trong quy trình xác minh phiếu. Chaincode tự tính lại gốc Merkle từ lá (leaf) sinh từ \pkg{blindedCommitment} cùng đường dẫn chứng minh do bên gọi cung cấp, sau đó so sánh với node gốc đã được cố định trên blockchain để khẳng định phiếu có thuộc cuộc bầu cử hay không.
...
Hàm \pkg{GetAuditCounts} thuộc loại \textit{query}, trả về đối tượng \pkg{AuditCountsView} chứa tổng số phiếu đã nộp, tổng số phiếu đã giải mù, cờ cho biết gốc Merkle đã được cố định hay chưa và cờ so khớp hai giá trị này. Hàm phục vụ chức năng kiểm toán đối chiếu số liệu giữa cơ sở dữ liệu và blockchain.
...
Hàm \pkg{RevealVoteCompact} thuộc loại \textit{invoke}, đóng vai trò trung tâm trong giai đoạn kiểm phiếu. Chaincode kiểm tra gốc Merkle đã được cố định, đối chiếu \pkg{revealKey} chưa từng xuất hiện trong tập \pkg{usedReveal} (chống phát lại trên chuỗi), sau đó lưu khóa giải mù kèm danh sách ứng viên được chọn, tăng bộ đếm \pkg{tally} cho mỗi ứng viên trong danh sách và tăng \pkg{RevealCount} đúng một lần cho mỗi lá phiếu (không phụ thuộc vào số ứng viên được chọn).
