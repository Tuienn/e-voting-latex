...
Chức năng \textit{xem thông tin tài khoản cá nhân} cho phép người dùng đã đăng nhập xem hồ sơ của chính mình. Định danh người dùng được xác thực ở tầng BFF, nhằm bảo đảm rằng không một tài khoản nào có thể đọc hồ sơ của tài khoản khác thông qua chức năng này.
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \begin{enumerate}
        ...
        \item BFF Gateway xác thực token và lấy định danh người dùng.
    ...
    Luồng sự kiện thay thế &
    \begin{itemize}
        \item 2a. Access token thiếu hoặc không hợp lệ: BFF Gateway từ chối yêu cầu ngay tại tầng xác thực, không chuyển tiếp tới Identity Service.
    \end{itemize} \\
    \hline
\end{longtable}
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \begin{itemize}
        \item 1a. Dữ liệu đầu vào không hợp lệ: BFF Gateway từ chối yêu cầu, không gọi tới Identity Service.
        \item 3a. Email mới trùng với tài khoản khác: Identity Service từ chối cập nhật.
        \item 3b. Tài khoản không tồn tại: Identity Service trả về lỗi không tìm thấy.
    \end{itemize} \\
    \hline
\end{longtable}
...
Chức năng \textit{tạo cuộc bầu cử} là điểm khởi đầu của vòng đời một cuộc bầu cử. Quản trị viên cần chỉ định tên duy nhất, danh sách ứng viên ban đầu và số ứng viên tối đa mỗi cử tri được chọn trên một lá phiếu (mặc định bằng $1$); cuộc bầu cử được khởi tạo ở trạng thái chờ và sẽ chỉ trở nên khả dụng cho cử tri sau khi được điều hành chuyển sang trạng thái hoạt động.
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \end{itemize} \\
    \hline
    Điều kiện sau & Một cuộc bầu cử mới được tạo trong cơ sở dữ liệu Coordinator với trạng thái chờ, danh sách ứng viên ban đầu và số ứng viên tối đa được chọn mỗi lá phiếu. \\
    \hline
    Luồng sự kiện chính &
    \begin{enumerate}
        ...
        \item Coordinator Service kiểm tra ràng buộc tên duy nhất và tính hợp lệ của số ứng viên tối đa được chọn, sau đó lưu bản ghi cuộc bầu cử với trạng thái chờ.
...
Chức năng \textit{quản lý ứng viên trong cuộc bầu cử} cho phép quản trị viên thêm hoặc xóa ứng viên khỏi danh sách của một cuộc bầu cử cụ thể. Để tránh thay đổi cấu trúc cuộc bầu cử sau khi cử tri đã bắt đầu bỏ phiếu, chức năng này chỉ áp dụng cho cuộc bầu cử đang ở trạng thái chờ.
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \hline
    \endlastfoot
    Tên chức năng & Thêm hoặc xóa ứng viên trong cuộc bầu cử ở trạng thái chờ. \\
    \hline
    Tác nhân & Quản trị viên. \\
    \hline
    Điều kiện trước &
    \begin{itemize}
        \item Đã đăng nhập với vai trò quản trị viên.
        \item Cuộc bầu cử ở trạng thái chờ.
        \item Các định danh đầu vào đều thuộc vai trò Ứng viên và đang ở trạng thái kích hoạt.
    \end{itemize} \\
    \hline
    ...
    \begin{enumerate}
        ...
        \item Coordinator Service mở giao dịch cơ sở dữ liệu, xác nhận lại trạng thái chờ của cuộc bầu cử rồi thêm hoặc xóa các bản ghi.
    ...
    \begin{itemize}
        \item 1a. Dữ liệu đầu vào không hợp lệ: BFF Gateway từ chối yêu cầu, không gọi tới Coordinator Service.
        \item 3a. Cuộc bầu cử không ở trạng thái chờ: Coordinator Service từ chối thao tác.
...
Chức năng \textit{quản lý cử tri trong cuộc bầu cử} cho phép quản trị viên cập nhật danh sách cử tri hợp lệ của một cuộc bầu cử. Vì danh sách cử tri quyết định ai có thể tham gia bỏ phiếu, hệ thống chỉ cho phép sửa đổi khi cuộc bầu cử còn ở trạng thái chờ.
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \hline
    \endlastfoot
    Tên chức năng & Thêm hoặc xóa cử tri trong cuộc bầu cử ở trạng thái chờ. \\
    \hline
    Tác nhân & Quản trị viên. \\
    \hline
    Điều kiện trước &
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \begin{itemize}
        \item Đã đăng nhập với vai trò quản trị viên.
        \item Khi bắt đầu: cuộc bầu cử ở trạng thái chờ, có tối thiểu hai ứng viên và ba cử tri.
        \item Khi đóng: cuộc bầu cử đang ở trạng thái hoạt động.
    \end{itemize} \\
    \hline
    Điều kiện sau &
    \begin{itemize}
        \item Khi bắt đầu thành công: cuộc bầu cử chuyển sang trạng thái hoạt động và lưu khóa công khai tập thể EC-Schnorr.
    ...
    \begin{enumerate}
        ...
        \item (Đóng) Coordinator Service đọc toàn bộ cam kết phiếu, xây dựng cây Merkle, sau đó gửi yêu cầu ghi gốc Merkle lên Mạng blockchain qua Chainlaunch.
    ...
    \begin{itemize}
        ...
        \item 3b. Một Signing Node phản hồi lỗi khi sinh khóa: Coordinator Service hủy thao tác và giữ cuộc bầu cử ở trạng thái chờ.
        \item 5a. Mạng blockchain phản hồi lỗi hoặc số lượng phiếu không khớp: Coordinator Service hủy chuyển trạng thái và giữ cuộc bầu cử ở trạng thái hoạt động.
    \end{itemize} \\
    \hline
\end{longtable}
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    Luồng sự kiện thay thế &
    \begin{itemize}
        \item 3a. Người dùng không có cuộc bầu cử nào liên kết: Coordinator Service trả về danh sách rỗng, không phát sinh lỗi.
    \end{itemize} \\
    \hline
\end{longtable}
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    Luồng sự kiện thay thế &
    \begin{itemize}
        \item 1a. Tham số lọc theo trạng thái không hợp lệ: BFF Gateway từ chối yêu cầu.
    \end{itemize} \\
    \hline
\end{longtable}
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    \begin{itemize}
        ...
        \item 5a. Một Signing Node phát hiện vi phạm ràng buộc duy nhất khi ghi bản ghi đã ký: giao dịch nội bộ của node bị hủy và yêu cầu ký toàn bộ bị từ chối.
        \item 8a. Mạng blockchain phản hồi lỗi khi ghi phiếu: Coordinator Service hủy lưu bản ghi phiếu và trả về lỗi cho cử tri.
    \end{itemize} \\
    \hline
\end{longtable}
...
\begin{longtable}{|p{0.22\textwidth}|p{0.72\textwidth}|}
    ...
    Luồng sự kiện thay thế &
    \begin{itemize}
        \item 3a. Danh sách ứng viên không hợp lệ (vượt quá số tối đa cho phép hoặc chứa ứng viên không thuộc cuộc bầu cử): Reveal-Vote từ chối tiết lộ.
        \item 3b. Chữ ký EC-Schnorr không hợp lệ: Reveal-Vote từ chối tiết lộ.
        \item 4a. Khóa giải mù đã tồn tại trong cơ sở dữ liệu hoặc trên blockchain: yêu cầu bị từ chối nhằm chống tấn công lặp.
        \item 4b. Mạng blockchain phản hồi lỗi: Reveal-Vote hủy lưu bản ghi và trả về lỗi.
    \end{itemize} \\
    \hline
\end{longtable}
