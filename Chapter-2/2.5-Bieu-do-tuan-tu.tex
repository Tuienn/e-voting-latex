...
Bước \textit{Xác thực và Kiểm tra quyền} xuất hiện lặp lại trong hầu hết các luồng yêu cầu xác thực; do đó được tách thành một biểu đồ tham chiếu dùng chung (Hình~\ref{fig:seq-ref-auth}) và được tham chiếu dưới dạng \pkg{ref} trong các biểu đồ còn lại nhằm tránh trùng lặp. Biểu đồ này mô tả cách BFF Gateway kiểm tra access token, trích xuất định danh người dùng và đối chiếu vai trò yêu cầu trước khi cho phép xử lý tiếp. Các nhánh lỗi như thiếu token, token hết hạn hoặc vai trò không phù hợp đều bị chặn ngay tại tầng BFF.
...
Biểu đồ tuần tự này mô tả thao tác thêm hoặc xóa ứng viên khỏi một cuộc bầu cử cụ thể. Coordinator Service kiểm tra trạng thái \pkg{PENDING} của cuộc bầu cử, sau đó gọi Identity Service để xác minh danh sách tài khoản ứng viên. Trước khi thực hiện ghi, Coordinator Service xác nhận lại trạng thái \pkg{PENDING} một lần nữa nhằm đảm bảo tính nhất quán trong môi trường đồng thời; danh sách ứng viên chỉ được cập nhật khi cả hai kiểm tra đều thông qua.
...
Biểu đồ tuần tự của chức năng quản lý cử tri mô tả cách quản trị viên thêm hoặc xóa cử tri khỏi danh sách được phép tham gia một cuộc bầu cử. Coordinator Service trước tiên gọi Identity Service để xác minh danh sách tài khoản cử tri; sau khi danh sách hợp lệ, Coordinator Service mở giao dịch ghi và kiểm tra lại trạng thái \pkg{PENDING} bên trong giao dịch để ngăn xung đột khi trạng thái cuộc bầu cử thay đổi đồng thời. Nếu trạng thái thay đổi trong khi giao dịch đang mở, giao dịch bị rollback và hệ thống trả về lỗi xung đột.
...
Biểu đồ tuần tự của chức năng điều hành cuộc bầu cử được tách thành hai luồng riêng. Hình~\ref{fig:seq-dieu-hanh-cuoc-bau-cu-bat-dau} mô tả luồng bắt đầu: Coordinator Service kiểm tra điều kiện cấu hình và phối hợp với ba Signing Node sinh khóa công khai tập thể EC-Schnorr trước khi chuyển trạng thái sang \pkg{ACTIVE}. Hình~\ref{fig:seq-dieu-hanh-cuoc-bau-cu-dong} mô tả luồng đóng: hệ thống xây dựng cây Merkle từ toàn bộ cam kết phiếu đã xác nhận, ghi trạng thái trung gian \pkg{CLOSING} vào cơ sở dữ liệu trước khi cam kết gốc Merkle lên blockchain, và xử lý phục hồi trong trường hợp partial-fail.
