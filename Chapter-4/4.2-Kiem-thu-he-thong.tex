...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|L{2}|C{0.75}|C{0.75}|C{0.75}|C{0.75}|}
        ...
        & $p_{50}$ & $p_{95}$ & $p_{50}$ & $p_{95}$ \\
        \hline
        \pkg{sign-in}              &  75    &  89    & 116   & 280   \\
        \hline
        \pkg{start-session}        &  22    &  26    &  43   & 100   \\
        \hline
        \pkg{sign (threshold)}     &  15    &  19    &  26   &  68   \\
        \hline
        \pkg{submit (blockchain)} & 2\,088 & 2\,132 & 487 & 1\,352 \\
        \hline
    \end{tabularx}
\end{table}

Ba bước đầu (\pkg{sign-in}, \pkg{start-session}, \pkg{sign (threshold)}) có độ trễ thấp và ổn định (15--116~ms). Toàn bộ \emph{nút cổ chai} tập trung ở bước \pkg{submit (blockchain)}: $p_{50}$ = 2\,088~ms ở P1 so với 487~ms ở P3. Sự chênh lệch này phản ánh cơ chế batch của orderer -- ở tải thấp (P1/P2), block bị cắt sớm theo \pkg{BatchTimeout} khoảng 2s trước khi đầy giao dịch; ở tải cao (P3/P4), các giao dịch đến đủ nhanh để lấp đầy block trước khi hết timeout, rút thời gian chờ xuống đáng kể. Ở P4 (1.000 cử tri), $p_{50}$ của bước \pkg{submit} tiếp tục giảm xuống còn khoảng 410~ms nhờ mật độ giao dịch cao hơn, trong khi $p_{95}$ tăng nhẹ do hàng đợi tại tầng endorsement.
...
\begin{table}[H]
    ...
    \begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}|p{5.5cm}|p{4.5cm}|c|c|}
        ...
        \hline
        Bỏ phiếu hai lần (\pkg{double-vote})            & \pkg{409 Already voted}       & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Nộp phiếu chữ ký giả mạo                        & \pkg{409 Signature mismatch}  & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Bỏ phiếu sau khi bầu cử đóng                    & \pkg{400 Election not active} & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Tiết lộ phiếu chữ ký giả mạo                    & \pkg{400 Invalid signature}   & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Tiết lộ ứng viên ngoài danh sách                & \pkg{403 Candidate not in election} & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Tiết lộ phiếu trùng lặp (\pkg{double-reveal})   & \pkg{409 Already revealed}    & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Replay nonce ký mù đã sử dụng                   & \pkg{409 Nonce already used}  & \pkg{PASS} & \pkg{PASS} \\
        \hline
        Truy xuất API quản trị không có quyền            & \pkg{401 Unauthorized}        & \pkg{PASS} & \pkg{PASS} \\
        \hline
...
Toàn bộ tám kịch bản vi phạm đều bị từ chối đúng như thiết kế ở cả hai pha. Cơ chế kiểm tra trạng thái phiên Redis kết hợp với chỉ mục duy nhất MongoDB trên hai định danh \pkg{electionId} và \pkg{voterId} đảm bảo các nỗ lực \pkg{double-vote} hoặc \pkg{double-reveal} đều bị phát hiện tại tầng dịch vụ trước khi yêu cầu đến blockchain. Tấn công replay nonce bị chặn nhờ danh sách đen nonce phía server; truy xuất trái phép API quản trị bị từ chối bởi middleware xác thực JWT hai cặp khóa.
...
\begin{itemize}
    ...
    \item \textbf{Hành vi batch của orderer:} Ở tải thấp (P1/P2/P5), phần lớn block chỉ chứa 1--2 giao dịch do block bị cắt theo \pkg{BatchTimeout} khoảng 2s trước khi đủ số lượng tối đa; ở tải cao (P3/P4), các giao dịch đến đủ nhanh để lấp đầy block trước khi hết timeout -- interval median giảm từ 6{,}33s (P1) xuống 0{,}87s (P3) và 0{,}72s (P4), throughput tăng tương ứng lên 6{,}90~tx/s.
    \item \textbf{Khả năng mở rộng từ 500 lên 1.000 cử tri:} Khi tải nhân đôi từ P3 lên P4, throughput tăng từ 4{,}98~tx/s lên 6{,}90~tx/s; $p_{50}$ của bước \pkg{VOTE} giảm từ 755~ms xuống 638~ms nhờ mật độ giao dịch cao hơn lấp đầy block sớm hơn, trong khi $p_{95}$ tăng nhẹ từ 1{,}571~ms lên 1{,}742~ms do hàng đợi tại tầng peer endorsement -- hệ thống vẫn vận hành ổn định, không có lỗi.
\end{itemize}
